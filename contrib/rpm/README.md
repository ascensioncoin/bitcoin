...
The RPM spec file provided here is for Ascension-Core 0.12.0 and builds on CentOS
...
[Ascension git repository](https://github.com/ascensioncoin/AscensionCoin) but are not part of
...
Currently only `Source10` is used, for the example `ascension.conf` file.
...
in the ascension git repository. At present only an SVG version of the Ascension
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the ascension
...
needed when 0.12.1 is released, a proper fix is already in the Ascension git
master branch.
...
idea to build Ascension Core against a static version of that library compiled
...
The `ascension.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `ascension-server` package. At this point, most RPM based
...
The `ascension-server` package should have SELinux support. How to properly do
...
Ascension-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/ascension/). He has been
packaging Ascension for Fedora at least since 2012.
...
4. I renamed the `ascension` package that contains the Qt GUI to `ascension-core` as
that appears to be how the general population refers to it, in contrast to
`ascension-xt` or `ascension-classic`. I wanted to make sure the general population
