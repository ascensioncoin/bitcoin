...
*Setup instructions for a Gitian build of Ascension Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Ascension
...
- [Building Ascension Core](#building-ascension-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-ascension-host/README.md).
...
Clone the git repositories for ascension and Gitian.
...
in the ascension repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Ascension Core
----------------

To build Ascension Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ascension repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ascension/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ascensioncoin/AscensionCoin
...
For example:
```bash
URL=https://github.com/laanwj/ascension.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ascension=${COMMIT} --url ascension=${URL} ../ascension/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ascension=${COMMIT} --url ascension=${URL} ../ascension/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ascension=${COMMIT} --url ascension=${URL} ../ascension/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the ascension git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ascension/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/ascension-core/ascension-detached-sigs.git

ASNPATH=/some/root/path/ascension
SIGPATH=/some/root/path/ascension-detached-sigs

./bin/gbuild --url ascension=${ASNPATH},signature=${SIGPATH} ../ascension/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ascension-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ascension-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ascension-osx-build.assert
```
...
[ascension-core/gitian.sigs](https://github.com/ascension-core/gitian.sigs/) repository, or if that's not possible create a pull
